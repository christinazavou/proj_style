...
#include "opencv2/core/types_c.h"  // cvSize is moved here in opencv version 4
#include <cmath>
#include <cstring>
...
static constexpr const char PATH_SEP_C = '/';
...
void initialize(string params)
{
    ...
    {
        mkdir(patch_path.data(),0777);
    }
    if (access(seed_path.c_str(), F_OK) == -1)
    {
        mkdir(seed_path.data(), 0777);
    }
...
void render()
{
    ...
    for (int k = 0; k < fz.files.size(); k++)
    {
        ...
        int pos = fz.files[model_current].path.find_last_of(PATH_SEP_C);
...
void normalize()
{
    ...
    for (int k = 0; k < fz.files.size(); k++)
    {
        ...
        string fix_filename = fz.files[model_current].path.substr(0, pos) + PATH_SEP + temp;
